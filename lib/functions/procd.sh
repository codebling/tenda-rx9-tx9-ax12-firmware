...
procd_lock() {
	...
	if [ "$?" != "0" ]; then
		exec 1000>"$IPKG_INSTROOT/var/lock/procd/procd_${service_name}.lock"
		flock 1000
		if [ "$?" != "0" ]; then
...
_procd_set_param() {
	...
	case "$type" in
		...
		error)
			...
			json_close_array
		;;
		nice|term_timeout|stdout|stderr)
			json_add_int "$type" "$1"
		;;
		reload_signal)
			json_add_int "$type" $(kill -l "$1")
		;;
		pidfile|user|group|seccomp|capabilities|facility|pseudofs_acl)
