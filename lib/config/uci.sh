...
uci_set() {
	...
	uci_changes_before=`$UCI_APP changes`
	$UCI_APP ${UCI_CONFIG_DIR:+-c $UCI_CONFIG_DIR} set "$PACKAGE.$CONFIG.$OPTION=$VALUE"
	uci_changes_after=`$UCI_APP changes`
	if [ "${uci_changes_after}" != "${uci_changes_before}" ] && [ "${PACKAGE}" == "wireless" ]; then
		eval '$UCI_APP set meta-wireless.${CONFIG}.param_changed=1'
	fi
}
...
uci_remove() {
	...
	if [ "${PACKAGE}" == "wireless" ]; then
		if [ -z ${OPTION} ]; then # >> delete the whole interface
			tmp_device=`$UCI_APP get $PACKAGE.$CONFIG.device`
			eval '$UCI_APP set meta-wireless.${tmp_device}.interface_changed=1'
			eval '$UCI_APP delete meta-wireless.${CONFIG}'
			...
			uci_changes_after=`$UCI_APP changes`
			if [ "${uci_changes_after}" != "${uci_changes_before}" ]; then
				eval '$UCI_APP set meta-wireless.${CONFIG}.param_changed=1'
			fi
		fi
	else
...
uci_commit() {
	local PACKAGE="$1"
	eval '$UCI_APP -c /tmp/ commit meta-wireless'
	$UCI_APP ${UCI_CONFIG_DIR:+-c $UCI_CONFIG_DIR} commit "$PACKAGE"
}
...
uci_add_vap() {
	...
	local VALUE="$4"
	if [ -z ${VALUE} ]; then
		eval '$UCI_APP set meta-wireless.${CONFIG}=${OPTION}'
		$UCI_APP ${UCI_CONFIG_DIR:+-c $UCI_CONFIG_DIR} set "$PACKAGE.$CONFIG=$OPTION"
	else
		eval '$UCI_APP set meta-wireless.${CONFIG}.${OPTION}=${VALUE}'
		eval '$UCI_APP set meta-wireless.${VALUE}.interface_changed=1'
		eval '$UCI_APP set meta-wireless.${CONFIG}.param_changed=1'
		$UCI_APP ${UCI_CONFIG_DIR:+-c $UCI_CONFIG_DIR} set "$PACKAGE.$CONFIG.$OPTION=$VALUE"
	fi
}
...
function wireless_apply() {
	out_tmp=`$UCI_APP changes`
	if [ ! -z "${out_tmp}" ]
	then
		#Cache not empty
		echo "ERROR: Cache not empty, please commit your changes or free the cache"
		echo "**To free cache, Use: uci revert wireless"
		exit
	else
		merge
		uci_commit "wireless"
...
function merge() {
	radios=`grep config /tmp/meta-wireless | grep -o "'.*'" | sed "s/'//g"`
	#grep config /tmp/meta-wireless : grep all the interfaces lines
	#grep -o "'.*'"                                 : grep only the intefaces names
	#sed "s/'//g"                                   : clear the single quotes
	for line in $radios; do
		pchanged=`eval '$UCI_APP get meta-wireless.${line}.param_changed'`
		if [ "${line/#radio*}" != "$line" ]; then
			ichanged=`eval '$UCI_APP get meta-wireless.${line}.interface_changed'`
			eval '$UCI_APP set wireless.${line}.interface_changed=${ichanged}'
		fi
		eval '$UCI_APP set wireless.${line}.param_changed=${pchanged}'
	done
}

function reset_w_func() {
	radios=`grep config /tmp/meta-wireless | grep -o "'.*'" | sed "s/'//g"`
	for line in $radios; do
		eval '$UCI_APP del wireless.${line}.param_changed'
		if [ "${line/#radio*}" != "$line" ]; then
			eval '$UCI_APP del wireless.${line}.interface_changed'
		fi
	done
}

function reset_mw_func() {
	radios=`grep config /tmp/meta-wireless | grep -o "'.*'" | sed "s/'//g"`
	for line in $radios; do
		eval '$UCI_APP set meta-wireless.${line}.param_changed=0'
		if [ "${line/#radio*}" != "$line" ]; then
			eval '$UCI_APP set meta-wireless.${line}.interface_changed=0'
		fi
	done
}
...
function main_func() {
	if [ "$1" = "set" ]; then
		...
		$UCI_APP revert wireless; $UCI_APP revert meta-wireless
	else
