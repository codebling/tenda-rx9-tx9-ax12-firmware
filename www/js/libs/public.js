...
function showSaveMsg (num, str, flag, change,time) {
	var hideDialog = true;
	str = str || _("Saving...");
	flag = flag || "-1";
 ...
 * msg为string
 */
function showDFSMsg(num, flag, time, msg, callback) {
    if (num == 0) {
        flag = flag || '0';
        time = time || 60;
        var dfsTimeVal, dfsTimeOut;
        $(".main-dailog").addClass("none");
        clearInterval(dfsTimeVal);
        clearTimeout(dfsTimeOut);
        $("#gbx_overlay").remove();
        $("<div id='gbx_overlay'></div>").appendTo("body");
        $(".dfs-msg").removeClass("none");
       if (flag == '0') {
            msg = "遵照国家法律法规，路由器正在进行动态频率选择(DFS)，Wi-Fi信号需要<span style='color:#f00;'>70s</span>左右才能正常连接，请稍候…";
            time = 70;
            $('.dfs-time').html(time);
            $("#dfs-message").html(msg);
            dfsTimeVal = setInterval(function () {
                time--;
                $('.dfs-time').html(time);
                switch (time) {
                    case 0:
        ...
        dfsTimeOut = setTimeout(function () {
	...
	this.page = function (pageView, pageModel) {
		function PageLogic (pageView, pageModel) {
			...
			this.submit = function () { //提交数据接口
				...
				//TODO: 执行提交动作
				wrappedRequest(moduleRequest,pageModel,dataStr)
...
//解决iframe模块中的请求多次点击发送
function moduleRequest(pageModel,dataStr){
	$.post(pageModel.setUrl, dataStr, function (str) {
		pageModel.afterSubmit(str);
		moduleRequest.isRequest = false;
	});
}
function wrappedRequest(fun,pageModel,dataStr){
	if(fun.isRequest){
		return
	}
	fun.isRequest = true;
	fun(pageModel,dataStr);
}
