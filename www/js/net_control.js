...
function updateData(dataList) {
	var orgResData = JSON.parse(JSON.stringify(dataList));
	orgResData.shift();
	...
	drawList(initDataList,orgResData);
}
...
//用数据创建（更新）列表：table
function drawList(dataList,orgResData) {
	var rowData = {},
		addData = {},
		limitUp,
		limitDown,
	...
	if(macList.length!==0){
		var resMacList = [];
		var delMacList = [];
		for (var i = orgResData.length - 1; i >= 0; i--) {
			resMacList.push(orgResData[i].mac);
		}
		delMacList = macList.filter(function(item){
			return resMacList.indexOf(item) ===-1;
		})
		for(var i = 0; i < delMacList.length; i++){
			$("tr[alt='"+delMacList[i]+"']").remove();
		}
	}
