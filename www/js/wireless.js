...
var view = R.moduleView({
	initEvent: function () {
		...
		$("#adv_band").on("change", changeBandwidth);
	}
});
...
function getBandWidthList() {
    ...
    // 选802.11a时，把40/80/160兆频宽毙掉。选802.11n时把80/160兆毙掉
    var mode_5g_init = $('#adv_mode_5g').val();
    if (mode_5g_init === 'a only') {
        $("#adv_band_5g").html('<option value="20">20</option>').val("20");
    } else if (mode_5g_init === 'n only' && ("40" in G_data.channel_5g || "80" in G_data.channel_5g || "160" in G_data.channel_5g)) {
        $("#adv_band_5g").html('<option value="20">20</option><option value="40">40</option><option value="auto">20/40</option>');
    }
...
function changeBandwidth() {
	...
	bandwidth = (bandwidth == "auto" ? "40" : bandwidth);
	channel = G_data.channel[bandwidth];
	len = channel.length;
	for (i = 0; i < len; i++) {
		if (i == 0) {
