...
function initEvent() {
	...
	$("select").on("change", function () {
		...
		if($(this).val() == 'wpa3sae' || $(this).val() == 'wpa3saewpa2psk' || $(this).val() == 'none'){
			var infoMsg = $(this).val() == 'wpa3sae'? '请确认接入终端支持WPA3-SAE模式，使用中如果遇到设备连接问题，建议切回WPA/WPA2-PSK':
				($(this).val() == 'none'?'WiFi未加密，有被人蹭网的风险，建议设置WI-FI密码':'请确认接入终端支持WPA3-SAE/WPA2-PSK模式，使用中如果遇到设备连接问题，建议切回WPA/WPA2-PSK');
...
function initValue(obj) {
	...
	if (obj.security === "none") {
		...
		$("select").eq(0).after( $("<p style='color: #999999;margin-right: 130px;'>WI-FI不加密，有被人蹭网的风险，建议设置WI-FI密码</p>"))
	} else {
		...
		if(obj.security === 'wpa3sae'|| obj.security ==='wpa3saewpa2psk' ){
			var infoMsg = obj.security == 'wpa3sae'? '请确认接入终端支持WPA3-SAE模式，使用中如果遇到设备连接问题，建议切回WPA2-PSK':'请确认接入终端支持WPA3-SAE/WPA2-PSK模式，使用中如果遇到设备连接问题，建议切回WPA2-PSK';
			var $info = $("<p style='color: #999999;margin-right: 130px;'>"+infoMsg+"</p>");
	...
	if (obj.security_5g === "none") {
		...
		$("select").eq(1).after( $("<p style='color: #999999;margin-right: 130px;'>WI-FI不加密，有被人蹭网的风险，建议设置WI-FI密码</p>"))
	} else {
		...
		if(obj.security_5g === 'wpa3sae' || obj.security_5g ==='wpa3saewpa2psk'){
			var infoMsg = obj.security_5g == 'wpa3sae'? '请确认接入终端支持WPA3-SAE模式，使用中如果遇到设备连接问题，建议切回WPA2-PSK':'请确认接入终端支持WPA3-SAE/WPA2-PSK模式，使用中如果遇到设备连接问题，建议切回WPA2-PSK';
			var $info = $("<p style='color: #999999;margin-right: 130px;'>"+infoMsg+"</p>");
