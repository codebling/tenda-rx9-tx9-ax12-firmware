...
export PATH=/opt/intel/sbin:/opt/intel/usr/sbin:/opt/intel/bin:${PATH}
...
find_bridge_WANs () {
	...
	then
		...
		then
			...
			if [ "${iface::3}" = "nas" ]; then
				. /opt/intel/etc/switch_functions "dsl" 2> /dev/null
			elif [ "$iface" = "ptm0" ]; then
				. /opt/intel/etc/switch_functions "dsl" 2> /dev/null
			elif [ "$iface" = "eth1" ]; then
				. /opt/intel/etc/switch_functions "eth" 2> /dev/null
			fi
