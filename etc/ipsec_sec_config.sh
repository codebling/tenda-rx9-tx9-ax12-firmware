...
if [ $# -lt 18 ] ; then 
	...
	echo -en "\$19 = esn - enable/disable(1/0)\n"
	echo -en "\$20 = instance number, if not set then its add operation \n"
	
	echo -en "\nExample: \n\n"
	echo -en "$0 1 pppoe-wan10 tunnel1 abcde ikev2 10.10.200.2 192.168.1.0/24 172.16.12.0/24 aes192 sha1 modp1536 prfsha1 aes192 sha1 modp1536 60 100.100.34.2 wan10 1 0 \n \n"
...
if [ -z "${20}" ]; then 
	...
	uci set ipsec.@remote[$i].enc_esn=${19}
fi


uci commit
/opt/intel/etc/uci_to_ipsec_config_owrt.sh restart ${18}
